...
export class qrPolynomial {
  ...
  multiply(e: any) {
    ...
    for (let i = 0; i < this.getLength(); i += 1) {
      for (let j = 0; j < e.getLength(); j += 1) {
        num[i + j] ^= this.qrMath.gexp(this.qrMath.glog(this.getAt(i)) + this.qrMath.glog(e.getAt(j)));
      }
    }
  ...
  mod(e: any) {
    ...
    let ratio = this.qrMath.glog(this.getAt(0)) - this.qrMath.glog(e.getAt(0));
...
export class QRUtil {
  ...
  getLostPoint(qrcode: QrCode): number {
    let moduleCount: number = qrcode.getModuleCount();
...
export class QRRSBlock {
  ...
  getRsBlockTable = function (typeNumber: number, errorCorrectionLevel: number): any[] {
...
export class QrCode {
  ...
  createData(typeNumber: number, errorCorrectionLevel: number, dataList: any): any {
    let rsBlocks = new QRRSBlock(typeNumber, errorCorrectionLevel).getRSBlocks(typeNumber, errorCorrectionLevel);
    let buffer: qrBitBuffer = new qrBitBuffer();
    for (let i = 0; i < dataList.length; i += 1) {
      let data = dataList[i];
      buffer.put(data.getMode(), 4);
      buffer.put(data.getLength(), this.qrUtil.getLengthInBits(data.getMode(), typeNumber));
      data.write(buffer);
    }
  ...
  createSvgTag(cellSize: number, margin: any): string {
    ...
    let c: any, mc: any, r: any, mr: any, qrSvg: string = '', rect: any;
  ...
  createImgTag(cellSize: number, margin: any): any {
    ...
    let _this = this;
    let imgTag: CreateImgTag = new CreateImgTag(size, size, function (x: any, y: any) {
